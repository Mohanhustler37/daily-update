...
import './NewTaskCollapsable.scss';
...
import { getSearchedTasks } from "../NewTaskListingQuery"
...
class NewTaskCollapsable extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
...
sidebarSections = [
  {id: 1, heading: 'Tasks', addIcon: addIcon, tooltip:"Add Task", subheading: [
    ...
    {id: 1, name: 'All', count: '24', tooltip:"All Tasks", onClick: this.props.thisObj.handleClickOutsideBody},
    {id: 2, name: 'My Tasks', count: '12', tooltip:"My Tasks", onClick: this.props.thisObj.handleClickOutsideBody},
    {id: 3, name: 'Due Today', count: '6', tooltip:"Due today", onClick: this.props.thisObj.handleClickOutsideBody},
    {id: 4, name: 'On Priority', count: '6', tooltip:"On priority", onClick: this.props.thisObj.handleClickOutsideBody},
  ]},
  {id: 2, heading: 'status', addIcon: '', subheading: [
    ...
    {id: 3, heading: 'grouping', addIcon: '', subheading: [
      ...
      {id: 3, name: 'Ticket by company', count: '6', tooltip:"Company"},
...
getTaskByPriority(
  ...
  searchTicketHere= ()=>{
    ...
    getSearchedTasks(client,this.state.searchOption,parseInt(localStorage.getItem("companyId")),data=>{
        ...
        this.props.refreshData(data.data.searchTask)
    })
  ...
  newTaskListingCollapse=()=> (
    <>
      <div className='tsk-lstng-collapse-sidebar-header'><p>Support&nbsp;/&nbsp;<span>Tasks</span></p></div>
      <div className="innerl2searchsectiontask">
      <div className='tsk-lstng-collapse-sidebar-search-container'>
        <TextField value={this.state.searchOption} name="searchOption" onChange={this.changeHndler} className='tsk-lstng-sidebar-srch-field' placeholder='Search Tasks' variant="outlined" />
      ...
      <div className="tasks-scrollable-section">
      {
        this.sidebarSections.map(section => (
          <div className='tsk-lstng-sidebar-tsk-section'>
            <div className='tsk-lstng-section-header'>
              <span className='tsk-lstng-section-header-name'>{section.heading}</span> {section.addIcon ? 
              // <img src={section.addIcon} alt='addIcon' onClick={()=> this.props.thisObj.setState({isCTaskDrawerOpen: !this.props.thisObj.state.isCTaskDrawerOpen})}/> 
              <ArrowTooltip title={section.tooltip} placement="right"> 
              <img className="section-header-icon" src={section.addIcon} alt='addIcon'
                onClick={() => this.props.thisObj.setState({ isCreateEditTaskDrawerOpen: !this.props.thisObj.state.isCreateEditTaskDrawerOpen, isEditTask: false })} /> 
                </ArrowTooltip>: null} 
            </div>
            {
              section.subheading.map(subheading => (
                <ArrowTooltip title={subheading.tooltip} placement="right">
                  <div className={`tsks-lstng-section-elements ${section.id === this.state.headingId && subheading.id === this.state.subheadingId ? 'tsks-lstng-active-section-element' : null }`} onClick={()=> this.subheadingClickHandler(section,subheading)}>
                    <span className='tsks-lstng-element-name '>{subheading.name}</span>
                    <span className={`${section.id === this.state.headingId && subheading.id === this.state.subheadingId ? 'tsks-lstng-while-active-element-count' : 'tsks-lstng-while-non-element-count' }`}>{subheading.count}</span>
                  </div>
                </ArrowTooltip>
              ))
            }
  ...
  render(){
    return this.newTaskListingCollapse()
  }
}

export default NewTaskCollapsable;
